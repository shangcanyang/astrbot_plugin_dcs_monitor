...
- WebUI 位号管理页面（插件 Pages）：在网页上直接增删改监控点位，保存后立即生效
...
### 5. WebUI 位号管理页面（插件 Pages）

插件内置「位号管理」网页页面，无需手动编辑配置即可在浏览器中增删改监控点位：

1. 打开 AstrBot WebUI →「插件管理」→ 点击 `astrbot_plugin_dcs_monitor` 插件卡片进入详情页。
2. 点击页面上的「位号管理」入口打开管理页。
3. 在表单中填写位号、描述、阈值、检查间隔，点击「添加」即可新增点位；表格中可「编辑」「删除」已有点位。
...
| 版本 | 说明 |
|------|------|
| v2.6.0 | 新增插件 Pages「位号管理」网页页面：WebUI 内直接增删改点位，保存即持久化并热重启监控 |
| v2.5.0 | 点位独立调度轮询（各点位按自身 check_interval 互不影响）、预警目标持久化、修复版本与注册信息不一致，补充元数据规范字段 |
